"""Install six independently reviewed portraits; preserve existing identities and rollback."""
...
SOURCE=ROOT/'sources/judge_portrait_refocus_20260919/cand'
TARGET=ROOT/'sources/judge_presentation_20260918'
OUT=Path(__file__).resolve().parent/'portrait_integration'
...
def main():
    ...
    proposed=rows(SOURCE/'portraits.append.jsonl');assert len(proposed)==len(accepted)==6
    with writer_lock(judges.DB):
        ...
        try:
            ...
            assert summary['profiles']==10698 and summary['profiles_with_photos']==49 and summary['analysis_records']==708
            assert judges.listing({'has':'photo'})['total']==49
            for row in proposed:
    ...
    receipt={'status':'passed','integrated_at':datetime.now(timezone.utc).isoformat(),'new_portraits':len(staged),
             'profiles_with_photos':49,'browse_profiles':10698,'identity_merges_added':0,'separate_profiles_added':0,
             'held_unlinked_portraits':9,'manifest_sha256':sha(manifest),'database_sha256':sha(judges.DB),
