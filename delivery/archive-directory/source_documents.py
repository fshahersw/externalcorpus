"""Read-only, hash-gated adapter for documents downloaded from source-directory addresses (sources/source_directory_documents_20260919).
...
DATA = ROOT / 'sources/source_directory_documents_20260919'
COLLECTION = ROOT / 'corpus/source_directory_documents_20260919'
DB_NAME = 'documents.sqlite3'
ADAPTER = 'source_documents'
DEFAULT_LIMIT, MAX_LIMIT = 50, 100
READER_CHARS = 60000
_LOCK = threading.Lock()
_CACHE: dict = {}
FAMILY_LABELS = {'settlement_agreements': 'Settlement agreements & consent judgments', 'court_forms': 'Court forms', 'court_rules': 'Court rules', 'standing_orders': 'Standing orders',
                 'opinions_decisions': 'Opinions & decisions', 'efiling_cmecf': 'E-filing & CM/ECF guides', 'agency_guidance': 'Agency guidance', 'jury_instructions': 'Jury instructions',
                 'enforcement_actions': 'Enforcement actions', 'regulations_register': 'Regulations & registers', 'attorney_admission_discipline': 'Attorney admission & discipline',
                 'recall_safety_data': 'Recall & safety data', 'uncategorized': 'Other documents'}
...
def listing(params=None, folder=None):
    ...
    filters = [{'name': 'q', 'label': 'Search titles and document text', 'type': 'search', 'placeholder': 'e.g. consent judgment, opioid, Johnson, local rule, jury instruction'},
               {'name': 'agency', 'label': 'Document category', 'type': 'select', 'options': [{'value': f, 'label': FAMILY_LABELS.get(f, f), 'count': n} for f, n in families if f]},
    ...
    return {'available': True, 'total': total, 'page': page, 'limit': limit, 'qualification': state['qualification'], 'filters': filters,
            'columns': [{'key': 'title', 'label': 'Document'}, {'key': 'agency', 'label': 'Category'}, {'key': 'type', 'label': 'Type'}, {'key': 'size', 'label': 'Size'}, {'key': 'saved', 'label': 'Saved'}],
            'results': results}
...
def for_agency(family, folder=None):
    ...
    if not total:
        return None
    return {'total': total, 'results': [{'id': str(r['id']), 'title': r['title'], 'subtitle': r['url']} for r in rows], 'link': '#source-documents?agency=' + family,
            'qualification': state['qualification']}
