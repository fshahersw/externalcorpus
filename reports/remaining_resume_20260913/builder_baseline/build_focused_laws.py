...
def main():
    ...
    for source in bundle['resource_inventory.jsonl']:
        ...
        if r['source_url'] == wy['source_url']:
            ...
            if r['content_kind'] == 'law_chapter_body':
                r['whole_document_status'] = 'observed_full_chapter_not_whole_code'
        else:
    ...
    official_fields = ['source_url','jurisdiction','category','title','content_kind','body_status','whole_document_status',
        ...
        'source_contexts','captures','verified_texts','document_derivatives','ocr_screenings','classification_basis',
    ...
    summary = {'builder': relative(__file__), 'version': VERSION, 'started_at_utc': started, 'completed_at_utc': now(),
        ...
        'input_method': 'Fresh read-only original-law reconciliation using saved metadata and three law SQLite archives; retained index verification for unchanged artifacts; direct provider JSON/frontier and browser manifests for all Trellis laws. Not stale catalog-only counts.',
...
def write_count_lineage():
    ...
    text += f'\nCount lineage: the four base law index collections contain {len(selected):,} capture records / {len(grouped):,} distinct URLs. Five overlapping URLs explain the record/URL difference. This package adds {len(extras)} distinct URLs: one verified Nebraska parser repair and 12 preserved HTTP-200 source-entry representations without verified nonempty text. The latter are retained bytes with review/JavaScript/rendered status, not extra legal bodies. No base-index URL is omitted. Exact URLs, source metadata and overlap groups are in `provenance/index_count_reconciliation.json`.\n'
    (OUT / 'README.md').write_text(text, encoding='utf-8')
    write_json('files.sha256.json', {p.relative_to(OUT).as_posix(): {'sha256': digest(p.read_bytes()), 'bytes': p.stat().st_size}
        for p in sorted(OUT.rglob('*')) if p.is_file() and p.name != 'files.sha256.json'})
