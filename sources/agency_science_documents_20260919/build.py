"""Agency and science documents (2026-09-19): index of the files downloaded by corpus/agency_science_documents_20260919.

Offline snapshot of a growing download collection; re-run to pick up new files. Each row is one distinct file (by SHA-256)
fetched from an official agency or science host with the project's robots-respecting crawler. Original bytes and extracted
...
FTS_CHARS = 120_000
FAMILY_LABELS = {'atsdr': 'ATSDR (toxicological profiles)', 'epa': 'EPA', 'cpsc': 'CPSC', 'jpml': 'Judicial Panel on Multidistrict Litigation', 'ntp': 'National Toxicology Program',
                 'osha': 'OSHA', 'cdc': 'CDC', 'nhtsa': 'NHTSA', 'cms': 'CMS', 'fda': 'FDA'}
...
def main():
    ...
    for row in source.execute('SELECT rc.resource_id, c.source_family, c.seed_json FROM resource_contexts rc JOIN contexts c ON c.id=rc.context_id'):
        ...
        try:
            titles.setdefault(row['resource_id'], (json.loads(row['seed_json']) or {}).get('frontier_title'))
        except ValueError:
            pass
    if DB.exists():
        DB.unlink()
    ...
    for row in source.execute("SELECT * FROM resources WHERE status='downloaded' AND raw_complete=1 AND sha256 IS NOT NULL AND duplicate_of IS NULL ORDER BY id"):
        ...
        if extracted and not tool_name and extracted.lower() not in ('untitled', filename.lower()):
    ...
    validation = {
        ...
        'license_ref': 'us_government_public_documents_saved_copies', 'inputs': [{'path': (COLLECTION / 'corpus.sqlite3').as_posix(), 'note': 'live download collection; snapshot at build time'}],
    }
