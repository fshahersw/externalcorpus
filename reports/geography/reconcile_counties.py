...
def title_identity(title):
    ...
    # Bare "City" can be part of a county name (Charles City County,
    # James City County). Prefer an explicit entity suffix before treating
    # that word alone as an independent-city marker.
    match=re.match(r'^(.*?)\s+(City and Borough|Census Area|Planning Region|Independent City|County|Parish|Borough|Municipality)\b',title,re.I)
    if not match:match=re.match(r'^(.*?)\s+(City)\b',title,re.I)
    if match:
