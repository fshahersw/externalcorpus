...
COLLECTIONS = ['corpus/official_law_pages', 'corpus/official_law_recovery_pass_1', 'corpus/official_law_nebraska_chapters']
CATEGORIES = {'statutes', 'constitution', 'court_rules', 'home_rule_act', 'legislative_rules'}
INPUTS = {}
STAT_ISSUES = []
...
def main():
    ...
    for family in ('document_derivatives', 'xml_document_derivatives', 'epub_document_derivatives'):
        path = ROOT / 'corpus/official_law_pages' / family / 'manifest.jsonl'
    ...
    for r in resources.values():
        ...
        output = {'url': r['url'], 'contexts': [{'jurisdiction': j, 'category': c} for j, c in sorted(r['contexts'])],
            ...
            'content_role': role, 'classification_basis': basis, 'labels': labels,
    ...
    summary = {'snapshot_started_at_utc': started, 'snapshot_completed_at_utc': dt.datetime.now(dt.timezone.utc).isoformat(timespec='seconds'),
        'scope': 'Original official law map/captures; official-law pages; recovery; Nebraska chapters; completed document and OCR derivatives. Trellis/court/county collections excluded.',
