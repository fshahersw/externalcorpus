...
def main():
    ...
    ordered = sorted(candidates.values(), key=lambda r: (r['county_geoid'], ranks[r['category']], r['url']))
    ...
    selected_per_county = Counter()
    prioritized = sorted((record for records in eligible.values() for record in records), key=lambda record: (substantive_rank(record), record['county_geoid'] in previously_targeted_counties, record['county_geoid'], record['url']))
    for record in prioritized:
        if len(selected_urls) >= LIMIT:
            break
        if selected_per_county[record['county_geoid']] >= PER_COUNTY:
            continue
        if record['url'] in selected_urls:
            record['selection_status'] = 'deferred_shared_url_county_association_review'
            continue
        selected_per_county[record['county_geoid']] += 1
        record['selection_status'] = 'selected'
        record['selection_round'] = selected_per_county[record['county_geoid']]
        record['substantive_selection_basis'] = 'Observed direct-document URL and legal anchor/path; target content unreviewed until downloaded.'
    ...
    summary.update(previously_targeted_counties=len(previously_targeted_counties), selected_previously_untargeted_counties=len(set(counts) - previously_targeted_counties), selected_previously_targeted_counties=len(set(counts) & previously_targeted_counties), config_merge=config_merge, selection_order='Substantive observed rule/order/form/code downloads first; untargeted county associations second, FIPS/URL tie-breakers; up to three per county.')
...
def is_document(url):
    return bool(re.search(r'\.(pdf|docx?|rtf)(?:$|/)|/documentcenter/(?:home/)?view/|/getmedia/', urlsplit(url).path.lower()))


def classify(url, anchor):
    if not is_document(url):
        return None
    value = normalize(anchor + ' ' + urlsplit(url).path)
    if re.search(r'\b(calendars?|agendas?|minutes|meetings?|events?|notices?|advertisements?|public hearings?|work sessions?|newsletters?|brochures?)\b', value):
        return None
    category = original_classify(url, anchor)
    if not category:
        return None
    if category[0] == 'local_rules':
        return category
    if category[0] == 'local_laws_codes':
        if re.search(r'\b(code enforcement|ordinance violation|ordinance complaint)\b', value):
            return None
        return category
    if re.search(r'\b(elections?|candidates?|business|tax|taxes|drivers?|vehicles?|commissioners?|permit|permits|registration)\b', value):
        return None
    if category[0] == 'court_forms_filing_documents' and re.search(r'\b(courts?|judicial|civil|criminal|probate|divorce|summons|petition|affidavit|guardianship|appeal|eviction|plea|bond)\b', value):
        return category
    return None


def substantive_rank(record):
    return {'local_rules': 0, 'court_forms_filing_documents': 1, 'local_laws_codes': 2}[record['category']]


if __name__ == '__main__':
    main()
    summary = json.loads((OUT / 'summary.json').read_text(encoding='utf-8'))
    candidates = [json.loads(line) for line in (OUT / 'all_candidates.jsonl').read_text(encoding='utf-8').splitlines()]
    eligible_statuses = {'selected', 'deferred_batch_limit_or_per_county_cap', 'deferred_shared_url_county_association_review'}
    eligible_rows = [row for row in candidates if row['selection_status'] in eligible_statuses]
    dump(OUT / 'substantive_preparation_review.json', {'base_preparer_path': rel(ROOT / 'scripts/prepare_county_local_documents_20260914.py'), 'base_preparer_sha256': sha((ROOT / 'scripts/prepare_county_local_documents_20260914.py').read_bytes()), 'isolated_preparer_path': rel(Path(__file__)), 'isolated_preparer_sha256': sha(Path(__file__).read_bytes()), 'eligible_document_url_county_pairs': len(eligible_rows), 'eligible_unique_document_urls_before_county_cap': len({row['url'] for row in eligible_rows}), 'eligible_county_associations': len({row['county_geoid'] for row in eligible_rows}), 'selected_source_parent_collections': dict(Counter(proof['collection'] for seed in [json.loads(line) for line in (OUT / 'seeds.jsonl').read_text(encoding='utf-8').splitlines()] for proof in seed['provenance'])), 'selection_basis': 'Exact observed direct-document path and legal text in anchor/path. File type and actual legal content remain unverified until downloaded.', 'excluded_now': ['General administration/clerk landings', 'Meeting calendars, agendas, minutes and notices', 'Tax/business/driver/election filing matches'], 'original_captured_parent_collections_include_county_local_documents': True, 'collection_mutations': 0, 'network_requests': 0})
